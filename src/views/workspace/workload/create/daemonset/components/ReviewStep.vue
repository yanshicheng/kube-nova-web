    ...
    <div v-show="activeView === 'summary'" class="config-summary">
      ...
      <ElCard class="summary-card">
        ...
        <ElDescriptions :column="2" border size="default">
          <ElDescriptionsItem label="DaemonSet 名称" label-class-name="desc-label" :span="2">
            <ElTag type="primary" size="large" effect="dark">{{ daemonSetName }}</ElTag>
          </ElDescriptionsItem>
      ...
      <ElCard class="summary-card">
        ...
        <ElDescriptions :column="2" border>
          <ElDescriptionsItem label="部署模式" :span="2">
            <ElTag type="success" size="large" effect="dark">每节点一个 Pod</ElTag>
          ...
          <ElDescriptionsItem
            v-if="strategyType === 'RollingUpdate'"
            label="最大不可用"
          >
            {{ daemonSetStore.updateStrategy.rollingUpdate?.maxUnavailable || 1 }}
          </ElDescriptionsItem>
          <ElDescriptionsItem label="保留历史版本">
            {{ daemonSetStore.revisionHistoryLimit }} 个
          </ElDescriptionsItem>
          <ElDescriptionsItem label="最小就绪秒数">
            {{ daemonSetStore.minReadySeconds }} 秒
          </ElDescriptionsItem>
        </ElDescriptions>

        <ElAlert type="success" :closable="false" show-icon style="margin-top: 16px">
          <template #title>✅ DaemonSet 部署模式</template>
          该工作负载将在集群的每个节点上运行一个 Pod 副本，适用于节点级别的守护进程
        </ElAlert>
      </ElCard>
      ...
      <ElCard class="summary-card">
        <template #header>
          <div class="card-header">
            <div class="header-left">
              <Cpu :size="18" />
              <span>预估资源使用（每个节点）</span>
            </div>
          </div>
        </template>
        <ElAlert type="info" :closable="false" show-icon style="margin-bottom: 16px">
          基于容器资源配置计算的每个节点的资源使用量
        </ElAlert>
        <ElDescriptions :column="2" border>
          <ElDescriptionsItem label="CPU 请求">
            <ElTag type="primary" size="large">{{ perNodeCpuRequest }}</ElTag>
          </ElDescriptionsItem>
          <ElDescriptionsItem label="CPU 限制">
            <ElTag type="danger" size="large">{{ perNodeCpuLimit }}</ElTag>
          </ElDescriptionsItem>
          <ElDescriptionsItem label="内存请求">
            <ElTag type="primary" size="large">{{ perNodeMemoryRequest }}</ElTag>
          </ElDescriptionsItem>
          <ElDescriptionsItem label="内存限制">
            <ElTag type="danger" size="large">{{ perNodeMemoryLimit }}</ElTag>
          </ElDescriptionsItem>
        </ElDescriptions>
      </ElCard>
    </div>
...
// 计算每节点资源
const calculatePerNodeResources = (field: string) => {
...
const perNodeCpuRequest = computed(() => {
  const total = calculatePerNodeResources('cpu-request')
  return total > 0 ? `${total.toFixed(2)} Core` : '-'
})

const perNodeCpuLimit = computed(() => {
  const total = calculatePerNodeResources('cpu-limit')
  return total > 0 ? `${total.toFixed(2)} Core` : '-'
})

const perNodeMemoryRequest = computed(() => {
  const total = calculatePerNodeResources('memory-request')
  return total > 0 ? `${total.toFixed(0)} Mi` : '-'
})

const perNodeMemoryLimit = computed(() => {
  const total = calculatePerNodeResources('memory-limit')
  return total > 0 ? `${total.toFixed(0)} Mi` : '-'
})
...
const yamlPreview = computed(() => {
  try {
    const daemonSet = {
      ...
      kind: 'DaemonSet',
      metadata: {
        name: daemonSetName.value,
        namespace: metadataStore.namespace
      },
      spec: {
        updateStrategy: daemonSetStore.updateStrategy
      }
    }
