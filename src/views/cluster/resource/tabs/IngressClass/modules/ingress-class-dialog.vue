  ...
  >
    ...
    <div class="dialog-toolbar">
      ...
      <ElPopover placement="bottom-start" :width="500" trigger="click">
        ...
        <div class="help-popover">
          <h4>IngressClass 使用说明</h4>
          <div class="help-content">
            <p><strong>什么是 IngressClass？</strong></p>
            <p>IngressClass 定义了 Ingress 控制器的类型，用于指定处理 Ingress 资源的控制器。</p>

            <p><strong>关键配置：</strong></p>
            <ul>
              <li><strong>控制器：</strong>必填，指定处理 Ingress 资源的控制器名称（如 k8s.io/ingress-nginx）</li>
              <li><strong>默认 IngressClass：</strong>设为默认后，未指定 IngressClass 的 Ingress 将使用此类</li>
              <li><strong>参数配置：</strong>（可选）用于传递额外配置给控制器</li>
            </ul>

            <p><strong>注意事项：</strong></p>
            <ul>
              <li>一个集群可以有多个 IngressClass，但只能有一个默认类</li>
              <li>确保对应的控制器已正确部署</li>
            </ul>
          </div>
        </div>
    ...
    <div v-show="editMode === 'yaml'" class="yaml-container">
      <YamlEditorPro
        v-model="yamlContent"
        :readonly="loading"
        :filename="`${formData.name || 'ingressclass'}.yaml`"
        :show-toolbar="true"
...
<script setup lang="ts">
  ...
  import IngressClassForm from './ingress-class-form.vue'
  import {
    createIngressClassApi,
    updateIngressClassApi,
    getIngressClassYamlApi
  } from '@/api/workload/cluster-resource'
  import {
    ingressClassYamlToForm,
    ingressClassFormToYaml,
    validateIngressClassForm,
    createEmptyIngressClassForm
  } from './type.ts'
  ...
  const formData = ref<IngressClassFormData>(createEmptyIngressClassForm())
  ...
  const dialogTitle = computed(() => {
    return props.dialogType === 'create'
      ? '创建 IngressClass'
      : `编辑 IngressClass: ${props.resourceName}`
  })
  ...
  watch(
    ...
    () => {
      if (editMode.value === 'form' && !isInitializing.value) {
        try {
          yamlContent.value = ingressClassFormToYaml(formData.value)
          convertError.value = ''
        } catch (error: any) {
  ...
  watch(editMode, (newMode, oldMode) => {
    ...
    if (newMode === 'form' && oldMode === 'yaml') {
      // YAML -> 表单：需要验证
      try {
        const parsed = ingressClassYamlToForm(yamlContent.value)
    ...
    async (val) => {
      if (val) {
        ...
        if (props.dialogType === 'edit' && props.resourceName) {
          ...
          formData.value = createEmptyIngressClassForm()
          yamlContent.value = ingressClassFormToYaml(formData.value)
        }
  ...
  const loadExistingYaml = async () => {
    try {
      ...
      const yaml = await getIngressClassYamlApi({
        clusterUuid: props.clusterUuid,
        name: props.resourceName!
      })

      yamlContent.value = yaml
      formData.value = ingressClassYamlToForm(yaml)
    } catch (error: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (editMode.value === 'yaml') {
      ...
      try {
        formData.value = ingressClassYamlToForm(yamlContent.value)
      } catch (error: any) {
    ...
    // 验证表单
    try {
      validateIngressClassForm(formData.value)
    } catch (error: any) {
    ...
    if (props.dialogType === 'edit') {
      try {
        await ElMessageBox.confirm(
          `确定要保存对 IngressClass "${formData.value.name}" 的修改吗？`,
          '确认修改',
          {
    ...
    loading.value = true
    try {
      const finalYaml = ingressClassFormToYaml(formData.value)
      const api = props.dialogType === 'create' ? createIngressClassApi : updateIngressClassApi
      await api({
        clusterUuid: props.clusterUuid,
        yamlStr: finalYaml
      })
