  ...
  >
    ...
    <div class="dialog-toolbar">
      ...
      <ElPopover placement="bottom-start" :width="500" trigger="click">
        ...
        <div class="help-popover">
          <h4>ClusterRoleBinding 使用说明</h4>
          <div class="help-content">
            <p><strong>什么是 ClusterRoleBinding？</strong></p>
            <p>
              ClusterRoleBinding 将集群级别的角色绑定到主体（用户、组或
              ServiceAccount），授予其相应的权限。
            </p>

            <p><strong>主体类型：</strong></p>
            <ul>
              <li><strong>User：</strong>用户账号，如 john@example.com</li>
              <li><strong>Group：</strong>用户组，如 system:masters</li>
              <li><strong>ServiceAccount：</strong>Pod 中运行的应用账号，必须指定命名空间</li>
            </ul>
    ...
    <div v-show="editMode === 'yaml'" class="yaml-container">
      <YamlEditorPro
        v-model="yamlContent"
        :readonly="loading"
        :filename="`${formData.name || 'clusterrolebinding'}.yaml`"
        :show-toolbar="true"
...
<script setup lang="ts">
  ...
  import ClusterRoleBindingForm from './cluster-role-binding-form.vue'
  import {
  ...
  import {
    clusterRoleBindingYamlToForm,
    clusterRoleBindingFormToYaml,
    validateClusterRoleBindingForm,
    createEmptyClusterRoleBindingForm
  } from './type.ts'
  ...
  const formData = ref<ClusterRoleBindingFormData>(createEmptyClusterRoleBindingForm())
  ...
  const dialogTitle = computed(() => {
    return props.dialogType === 'create'
      ...
      : `编辑 ClusterRoleBinding: ${props.resourceName}`
  })
  ...
  watch(
    ...
    () => {
      if (editMode.value === 'form' && !isInitializing.value) {
        try {
          yamlContent.value = clusterRoleBindingFormToYaml(formData.value)
          convertError.value = ''
        } catch (error: any) {
  ...
  watch(editMode, (newMode, oldMode) => {
    ...
    if (newMode === 'form' && oldMode === 'yaml') {
      // YAML -> 表单：需要验证
      try {
        const parsed = clusterRoleBindingYamlToForm(yamlContent.value)
    ...
    async (val) => {
      if (val) {
        ...
        if (props.dialogType === 'edit' && props.resourceName) {
          ...
          formData.value = createEmptyClusterRoleBindingForm()
          yamlContent.value = clusterRoleBindingFormToYaml(formData.value)
        }
  ...
  const loadExistingYaml = async () => {
    try {
      ...
      const yaml = await getClusterRoleBindingYamlApi({
        clusterUuid: props.clusterUuid,
        name: props.resourceName!
      })

      yamlContent.value = yaml
      formData.value = clusterRoleBindingYamlToForm(yaml)
    } catch (error: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (editMode.value === 'yaml') {
      ...
      try {
        formData.value = clusterRoleBindingYamlToForm(yamlContent.value)
      } catch (error: any) {
    ...
    // 验证表单
    try {
      validateClusterRoleBindingForm(formData.value)
    } catch (error: any) {
    ...
    if (props.dialogType === 'edit') {
      try {
        await ElMessageBox.confirm(
          `确定要保存对 ClusterRoleBinding "${formData.value.name}" 的修改吗？修改可能会影响主体的权限。`,
          '确认修改',
    ...
    loading.value = true
    try {
      const finalYaml = clusterRoleBindingFormToYaml(formData.value)
      const api =
        props.dialogType === 'create' ? createClusterRoleBindingApi : updateClusterRoleBindingApi
      await api({
        clusterUuid: props.clusterUuid,
        yamlStr: finalYaml
      })
